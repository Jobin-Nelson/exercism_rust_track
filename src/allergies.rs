...
impl Allergen {
    fn get_allergen(score: u32) -> Self {
        match score {
            1 => Self::Eggs,
            2 => Self::Peanuts,
            4 => Self::Shellfish,
...
impl Allergies {
    ...
    pub fn is_allergic_to(&self, allergen: &Allergen) -> bool {
        let allergen_score = 1 << (*allergen as u8);
        allergen_score & self.score != 0
    }
    ...
    pub fn allergies(&self) -> Vec<Allergen> {
        ...
        while score <= 128 {
            let allergen = Allergen::get_allergen(score);
...
mod tests {
    ...
    fn compare_allergy_vectors(expected: &[Allergen], actual: &[Allergen]) {
        ...
        if actual.len() != expected.len() {
            panic!("Allergy vectors are of different length\n expected {expected:?}\n got {actual:?}");
        }
    }
    ...
    #[test]
    fn allergic_to_something_else_but_not_eggs() {
        let allergies = Allergies::new(2);
        assert!(!allergies.is_allergic_to(&Allergen::Eggs));
    }
    #[test]
    fn allergic_to_everthing_eggs() {
        let allergies = Allergies::new(255);
        assert!(allergies.is_allergic_to(&Allergen::Eggs));
    }
    #[test]
    fn allergic_to_anything_peanuts() {
        let allergies = Allergies::new(2);
